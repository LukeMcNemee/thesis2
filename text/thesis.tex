...
TODO general WSN stuff (wide usecases, IoT) (footnote for WSN)

One of the goals of this thesis was to create multi-purpose WSN platform. Intentionaly multipurpose, alhough we were avare of the fact that such network cannot outperform a dedicated one in one specific task. We had multiple tasks in mind even before the construction of this network and many other appeared later, therefore dedicate one network to each of these tasks would not be a cost effective solution. Moreover, it is possible to create general purpose network will be at least comparable to the dedicated one in most of the usecases. The basic decision making and problem analysisn regarding building such network is described in chapter \ref{ch:analysis} and the actual construction of our testbed, together with description of developed management software  is in the chapter \ref{ch:testbed}.

Once such network has been created, with all the required software and  functionality, it was used for two very much different usecases. First usecase was research goal; we had promissing results from Mathur et.al. \cite{Mathur2008Rssi} but these were produced on devices capable of much more than our usual WSN node (Mathur et.al. used both FPGA-based developement platforms for 802.11 standart and laptops with Atheros WiFi cards). This is the reason why we decided to replicate these results using our WSN platform.

Although this novel approach is without any doubt usefull for any kind of devices, it does not fully show its potentil on powerfull devices like laptops, because these devices are perfecty fine using asymetric cryptography. At the same time there are definetely better sources of randomness than the fading of radio chanel. However, we can see potential for this kind of approach in the area of ad-hoc networks and therefore we have to consider the most basic device we can imagine. These devices will most likely consist of microprocessor and simple radio, usually without any support for well known standarts (e.g. 802.11 \cite{crow1997ieee}) the most we can expect is support for ZigBee specification \cite{alliance2006zigbee}.

Therefore we decided to run simmilar set of experiments, only using hardware comparable to a typical WSN node. The set of experiments was later expanded, because we realised, that our static network is not capable of producing any randomenss without additional disturbances; let it be jamming, movement of people or an other means. To this part of thesis is dedicated chapter \ref{ch:research}.

The second usecase of our network is related to the usually complicated first experience with WSN. To remedy this problem, the second usecase is educational one. The goal is to create ad-hoc network, that would be easy to use and would help to introduce various concepts used in WSNs. This usecase was demonstrated during seminars of PA197 (Secure Network Design) course and slightly modified versions of individual scenarios were used as marked assigments. Basic principles of this usecase, detailed descriptions of individual scenarios and specifics of use during seminars are described in chapter \ref{ch:edu}.
...
\chapter{Research use}\label{ch:research}
  ...
  \subsection{Quantization principle (bits from signal strength)}
    ...
    and contrary to that we have lossy quantization \cite{Mathur2008Rssi}. %TODO quant. is inherently lossy
    ...
    Also, there are few different measurements of the channel one can use to produce bits. One would be RSSI measurements on received messages, another measure %\TODO %from paper and CITE
  ...
  This application uses speacially modified JeeNode motes \ref{sec:hw}, because analog RSSI values are not available through provided connections to radio module. Fortunately, analog RSSI can be obtained through short jumper wire from radio module to the Arduino analog pin %CITE.
  ...
  Application is programmed in such way, that 1000 counter values are exchanged, then they are send out via the serial port and application is terminated. Intervals between individual messages should be as short as possible, so that the random disturbances in the radio channel would be captured on both nodes. Even though RSSI measurement are definitely not symetrical, using this approach we can expect them to be highly correlated %CITE.
...
Min entropy \cite{barker2012recommendation} is considered the most conservative estimate of unpredictability in information theory. To make an min entropy estimate of IID\footnote{Independend and identically distributed} source we can use simple calculation as follows, given the dataset with N samples:
...
It is important to note, that this calculation of min entropy estimate is only meaningful for IID sources \cite{barker2012recommendation}, for non-IID sources we have to use other means. Testing sources with dependencies with this min entropy calculation may result in overestimates and generally output without any meanigful use.

There are many test for estimating min entropy contained within output of non IID sources, most notably collision test, partial collection test or the compression test \cite{barker2012recommendation}. To us the most meaningfull one was the frequency test\cite{barker2012recommendation}, because of its ability to work on small amounts of data, larger amount of data only increase the accuracy, but the test can provide meaningful estimate on any amount of data provided.

\paragraph{Frequency test}\label{par:freq}
  On first sight, the calculation is simmilar to the original min entropy \ref{par:min}. The calculation is following, using $0.005$ as confidence level $\alpha$:
...
\hline
  ...
  Thus we leave out this option for either more advanced users (who might be albe to control the radio on the most elemental level, therefore defining own collision avoidance rules etc.) or users with more specialised hardware (e.g. software defined radio modules). %CITE
  ...
  The big question regarding this source is the predicatability of such data, because as we can see on graph of sample run %TODO add image
  ...
  Anyway, approximately 30 seconds to exchange values and then some minor time for calculations and exchange of two messages with identified excursions (which are negligible regarding the time).  From these 1000 values we can obtain about 25 bits of enthropy on average \ref{src:people}, therefore we can gather about 50 bits per minute.

  This is comparable to the original result 1 bit/sec of Mathur et.al. \cite{Mathur2008Rssi}, however we were capable of echanging way less messages per minute than the original proposal (our rate 1000 msgs/30 sec is not comparable to 4000 msgs/sec achieved by Mathur et.al.), probally due to the nature of used devices \ref{sec:hw}.
  \subsection{Possible errors}
  Question about reliability surely comes to mind. We have no direct guarantee, values and measurement on individual nodes are clearly not symetrical, we only expect them to be highly correlated.

  As we can see on the boxplots \ref{fig:errorsT} \ref{fig:errorsS}, the errors between nodes A and B are indeed scarce, therefore there is reasonable probbability, that the nodes A and B will be able to establish common key, using the obtained bit sequence from quantisation algorith \ref{subsec:quant}.
