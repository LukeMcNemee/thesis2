...
\chapter{Research use}
  \section{Keys from radio signal}
    ...
    Quantization princeple designed by Mathur et.al. CITE works as follows:
    ...
    \begin{enumerate}
      ...
      \item nodes then synchronize their measurement by exchanging couner values asciated with those messages, where signal strength measurements were asigned either one or zero bit values.
      \item those couner values that match on both nodes are expected to be excursions in the same direction and are used in the final outcome. 
    \end{enumerate}
