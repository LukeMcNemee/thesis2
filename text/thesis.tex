...
% questions - power, programming, permanent vs temporal etc.
  ...
  Some of these are independent decisions, some are quite closely related to another. Now we can elaborate on these questions a bit more and provide possible reasoning for each one.
  \subsection{Network purpose (\ref{enu:purpose})}
  \subsection{Device selection (\ref{enu:devices}) and (\ref{enu:type})}
  \subsection{Network lifespan (\ref{enu:life})}
