...
\chapter{Education use}\label{ch:edu}
  ...
  We extended this approach with a static html page. The automaticly executed scripts then can modify the contents of html source file, therefore giving the information about current state of the network. Using this approach we can publish information about current scenario, number of active nodes or time left to the next iteration of scenario.
  ...
  During the first week students acomplished to learn the basics of Arduino programming, basic transmissions and two hop communication with three devices. Students also learned basic troubleshooting skills, which are required while working with hardware platforms like Arduino.

  As a asignment for first week we selected first Edu-hoc scenario \ref{subsec:1st}, which was completed by 29 students out of 33 enroled. Some students submitted only small sample of captured messages, which was counted as partially correct solution (working application, but not enough captured messages). This scenario was considered succesful, because majority of the students was able to create a working application and eavesdrop messages.
  ...
  As a second assignment we selected modified third scenario \ref{subsec:3rd}. The first part stayed intact, the goal was to deliver a modified message. Easch student was assigned unique string to modify messages with. Once this message was delivered to the central node, it responded with a message containing a secret word. This word had to be presented in the correct solution and was unique for each student.

   Only 10 students out of 33 submitted their solutions to the assignment, 3 of these were correct and received full score. Others usually made mistakes during the solution and did not recovered the correct secret word, most of  them received partial score. We do not consider this excercise succesfull, nor we consider it a failure. It was possible to solve this assignment, nevertheless, many students did not even tried to do so. Reasons why were partly caused by too complicated assignmet, partly by students not having enough time to complete it.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\chapter{Future work}\label{ch:future}
Regarding the testbed construction, there are only minor improvements one could do. These are not critical to the testbed functionality in any way. We can alway benefit from more nodes present in the network, which would require expansion to another rooms. Also current situation requires manual detection of failures. Automatic detection of disconected node, preferably with notification system, would be appreciated extension to already present management software.

  \section{Keys from RSSI}
  We can see one clear goal to acomplish with this usecase. As has been mentioned in \ref{sec:eval}, we did not manage to collect large amount of data. Therefore this should be the primary goal. Gather enough data from individual experiments; at least megabytes of binary data from sequences of secret shared bits combined together.

  Once we obtain large amount of data for each setting, we can then analyse such data with statistical test suites \cite{rukhin2001statistical}. Also we can employ more tests to estimate entropy \cite{barker2012recommendation} within our data.

  If we manage to collect large amounts of data, it would be also usefull to compare various aproached to the quantization problem, since there are multiple solutions published \cite{Azimi-Sadjadi2010Fading} \cite{Jana2009Rssi} \cite{azimi2007robust} and each one can perform differently on our network. Since we collect complete measurements (not only the resulting bit sequences), it is easy to run aditional analysis on our data and compare results.
  ...
  \section{Edu-hoc}
  Educational part of this thesis can be extended in two directions. First, we can create more scenarios. As one of possible scenarios, which could be implemented we have sybil atack. This would require implementation of voting scheme, and task of such scenario then would be to alter the expected result of voting. Another scenario could employ relay attack. Goal for an attacker would be to create non existing link in the routing tree, and then utilise this link to his advantage.

  The other direction is related to the user interface. Currently we are able to display the current state of our network, but we cannot give direct feedback. We would like to have responsive web interface, where users could submit their solution to currently running scenario and receive imediate evaluation of their efforts. This is directly connected to fully automated evaluation of solutions, which is currently also not possible, but it would be a appreciated time saving addition on administrator side.

\chapter{Summary}
The first part of this thesis is about the construction of wireless sensor network. First, from theoretical point of view we decide the nature of our network and all the requirements. In detail we go through each decision, why is it important, advantages and disadvantages of either solution.

In the next part, our decisions regarding the network build are presented, together with reasoning for each of our decisions. This part also covers the build itself, together with description od JeeLink nodes, which were selected as a primary device for our network. In the last section of this part JeeTool is introduced as a software created for management of our network.

Thesis then presents two usecases for our new network. We discuss rather novel approach to key generation using radio channel fading. Theoretic principles of this approach are presented, together with results from our experiments. Although we had not gathered enough data from our experiments, data we have so faw show promising results. We have established, that static network without disturbances does not carry enough entropy. Therefore such network cannot produce almost any secret shared bits. However, if we ensure presence of disturbances, either from movement of people or mechanical obstructions of radio signal, it is possible to garher entropy from signal measurements. In addition, our experiments show, that these measurements are correlated on two participating nodes. Thus we can ustilise these measurements to produce shared keys between two devices.

The second presented usecase has an educational goal. We show our solution to learnig the basics of ad-hoc networking through scenario based approach. Each scenario presents a simple task; and individual scenarios have incrementaly higher difficulty. The goal of each is to exploit vulnerability in the network using own devices and then make modification to the source code to prevent such attack in the future. With partial success we employed this approach in seminars of PA197 (only 3 students completed the second task). 
