...
\chapter{Research use}\label{ch:research}
  ...
  \subsection{Quantization principle}
    ...
    Main difference between these is number of generated bits per original signal strength measurements. Lossless quantization produces bit value from every measurement of signal strength, which is useful for high-performance demands, but it requires guaranteed variance in the radio channel (e.g. the nodes are constantly moving, or the environment is changing) Otherwise, the resulting keys could possibly be very weak.

    Lossy quantization, on the other hand, does not have guaranteed output length per number of measured values, which can lead to a very limited length of output. However, this kind of quantization is expected to have better results in static environments \cite{Jana2009Rssi} because its nature is to drop such bits, that fail to differ from others.
